...
def train_val(config, model, train_loader, val_loader, criterion):
    ...
    for epoch in range(config.train.num_epochs):
        ...
        # Normalize HD score to [0,1] range using exponential decay
        hd_norm = np.exp(-val_metrics['hd']/100)
        # Use geometric mean for better balance between metrics
        combined_score = np.sqrt(val_metrics['dice'] * hd_norm)
        
        # Save best model
        if combined_score > max_score and epoch >= 80:
